...
#  Showing the Weighted graph representation
weighted_graph = {
    'S': {'A': 3, 'B': 1},
    'A': {'S': 3, 'B': 2, 'C': 2},
    'B': {'B': 1, 'A': 2, 'C': 3},
...
#The Heuristics for different nodes
heuristics = {
    'S': 7,
    'A': 5,
    'B': 7,
...
def bfs(graph, start, goal):
    ...
    while queue:
        ...
        if node == goal:
            return path, expanded_states, []  # Returning the empty list for the unexpanded states
    ...
    return None, expanded_states, []  # Returning the empty list for the unexpanded states
...
def dfs(graph, start, goal):
    ...
    while stack:
        ...
        expanded_states.append(node)
        if node == goal:
            return path, expanded_states, []  # Returning an empty list for unexpanded states
        if node not in visited:
            visited.add(node)
            neighbors = sorted(graph[node].keys())
    ...
    return None, expanded_states, []  # Returning the empty list for unexpanded states
...
def ucs(graph, start, goal):
    ...
    return None, expanded_states, []  # Returning an empty list for unexpanded states
...
# Print results for Greedy search
